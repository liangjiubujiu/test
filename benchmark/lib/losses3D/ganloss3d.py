...
def get_scheduler(optimizer, lr_policy='plateau'):
    if lr_policy == 'lambda':
        def lambda_rule(epoch):
            lr_l = 1.0 - max(0, epoch + 1 + 1 - 2) / float(1 + 1)
            return lr_l
...
class GANLoss(nn.Module):
    def __init__(self, gan_mode='dice', target_real_label=1.0, target_fake_label=0.0,
