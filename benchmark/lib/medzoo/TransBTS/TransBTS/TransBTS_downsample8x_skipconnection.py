...
class TransformerBTS(nn.Module):
    def __init__(
        ...
        if positional_encoding_type == "learned":
            self.position_encoding = LearnedPositionalEncoding(args,
...
#todo
        self.Unet = Unet(in_channels=1, base_channels=16, num_classes=args.classes)
    ...
    def encode(self, x):
        ...
        # print('position_encoding x shape', x.shape)
        x = self.pe_dropout(x)
        # print('pe_dropout', x.shape)
        # apply transformer
        x, intmd_x = self.transformer(x)
        # print('transformer x shape', x.shape)

        x = self.pre_head_ln(x)
        # print('pe_head_ln', x.shape)

        return x1_1, x2_1, x3_1, x4_1,x, intmd_x,out
    ...
    def forward(self, x, auxillary_output_layers=[1, 2, 3, 4]):
        ...
        x1_1, x2_1, x3_1, x4_1,encoder_output, intmd_encoder_outputs,out = self.encode(x)

        decoder_output = self.decode(
            x1_1, x2_1, x3_1, x4_1,encoder_output, intmd_encoder_outputs, out,auxillary_output_layers
        )
    ...
    def _reshape_output(self, x):
        # x = x.view(
        #     x.size(0),
        #     int(self.args.batchDim[0] / self.patch_dim),
        #     int(self.args.batchDim[1] / self.patch_dim),
        #     int(self.args.batchDim[2] / self.patch_dim),
        #     self.embedding_dim,
        # )
        x=x.view(x.size(0),1,16,16,512)
...
class BTS(TransformerBTS):
    def __init__(
        ...
        self.DeUp1 = DeUp_Cat(in_channels=self.embedding_dim//16, out_channels=self.embedding_dim//32)
        self.DeBlock1 = DeBlock(in_channels=self.embedding_dim//32)

        self.endconv = nn.Conv3d(self.embedding_dim // 32, args.classes, kernel_size=1)


    def decode(self, x1_1, x2_1, x3_1, x4_1,x, intmd_x, out,intmd_layers=[1, 2, 3, 4]):
        ...
        # print('reshape x8.shape', x8.shape)
        x8 = self.Enblock8_1(x8)
        # print('enblock8_1 x8 shape',x8.shape)
        # x8 = self.Enblock8_2(x8)
        # print('enblock8_2 x8 shape', x8.shape)
        y4 = self.DeUp4(x8, x4_1)  # (1, 64, 32, 32, 32)
        # print('deup y4 shape', y4.shape)
        y4 = self.DeBlock4(y4)
        # print('deblock y4 shape', y4.shape)
        y3 = self.DeUp3(y4, x3_1)  # (1, 32, 64, 64, 64)
        # print('deup3 y3 shape', y3.shape)
        y3 = self.DeBlock3(y3)
        # print('deblock y3 shape', y3.shape)
        y2 = self.DeUp2(y3, x2_1)  # (1, 16, 128, 128, 128)
        # print('deup2 y2 shape', y2.shape)
        y2 = self.DeBlock2(y2)
        # print('deblock y2 shape', y2.shape)

        y1 = self.DeUp1(y2, x1_1)  # (1, 16, 128, 128, 128)
        # print('deup2 y1 shape', y1.shape)
        y1 = self.DeBlock1(y1)
        # print('deblock y1 shape', y1.shape)

        y = self.endconv(y1)      # (1, 4, 128, 128, 128)
        # print('endcov shape', y.shape)
        y = self.Softmax(y)
...
def TransBTS(args,dataset='brats', _conv_repr=True, _pe_type="learned"):

    if dataset.lower() == 'brats':
        #todo
        img_dim = args.batchDim[0]
        num_classes = args.classes

    num_channels = args.inChannels
    patch_dim = 8
    aux_layers = [1, 2, 3, 4]
    model = BTS(args,
        img_dim,
